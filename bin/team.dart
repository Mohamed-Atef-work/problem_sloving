...
void team() {
  print("Pleas Enter The Number Of The Problems !!!!");
  final String? numString = stdin.readLineSync();
  final int num = int.parse(numString!);

  int howManySolvedProblems = 0;
  List<List<int>> problems = [];

  for (int i = 0; i < num; i++) {
    print("Who Can Solve The Problem ${(i + 1)}...........");
    final String? problemInput = stdin.readLineSync();
    final List<String> digitsString = problemInput!.split(" ");
    final List<int> digitsInt = List.generate(
      digitsString.length,
      (index) => int.parse(digitsString[index]),
    );

    print(digitsInt);

    problems.add(digitsInt);
  }

  for (int i = 0; i < problems.length; i++) {
    final int total = problems[i][0] + problems[i][1] + problems[i][2];
    if (total >= 2) {
      howManySolvedProblems++;
    }
  }
  print("Calculating...........................");

  print("You Can Solve ------------> [$howManySolvedProblems]");
}
